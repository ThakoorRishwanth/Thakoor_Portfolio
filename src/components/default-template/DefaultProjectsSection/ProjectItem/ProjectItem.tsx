...
export const ProjectItem = ({
  ...
  // Fix the URL by ensuring it is an absolute path
  const fixedUrl = url.startsWith("http") ? url : `https://${url}`;

  return (
    <div className="group animate-hidden relative flex flex-col gap-[20px] pb-[20px] p-[20px] bg-background2 rounded-lg shadow-lg hover:shadow-xl duration-300 transform transition-all hover:scale-105">
      {isPinned && (
        <div className="absolute top-[-8px] left-[-8px] text-primary z-5 pointer-events-none [&>svg]:w-[24px] [&>svg]:h-[24px] [&>svg]:rotate-[270deg]">
          <PinIcon />
        </div>
      ...
      <Link
        ...
        rel="noopener noreferrer"
      >
        <div className="relative max-w-full rounded-lg pb-[75%] overflow-hidden duration-200 shadow-[0px_4px_7px_theme(colors.primary)] group-hover:opacity-50 group-focus-visible/imgLink:shadow-[0px_0px_0px_2px_theme(colors.primary)]">
          <img
            className="absolute left-0 top-0 h-full w-full object-cover select-none duration-300 group-hover:scale-[1.015]"
            alt={title}
            src={image}
      ...
      <div className="flex flex-col gap-[12px] whitespace-pre-line text-white">
        {isPinned && (
          <Typography className="text-primary text-l" weight="bold">
            Pinned
          </Typography>
